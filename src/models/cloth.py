...
class cloth_config:
    ...
    pin_top_corners: bool = False
    pin_left_column: bool = False
    pin_right_column: bool = False
    pin_all_edges: bool = False

class cloth:
    def __init__(self, config: cloth_config, sim_params: dict) -> None:
        self.config = config
        ...
        for row in range(self.config.rows):
            for column in range(self.config.columns):
                ...
                if row == 0 and self.config.pin_top_row:
                    self.pinned.add(index)
                if (row == 0 and column == 0 and self.config.pin_top_corners) or \
                   (row == 0 and column == self.config.columns - 1 and self.config.pin_top_corners):
                    self.pinned.add(index)
                if column == 0 and self.config.pin_left_column:
                    self.pinned.add(index)
                if column == self.config.columns - 1 and self.config.pin_right_column:
                    self.pinned.add(index)
                if self.config.pin_all_edges and (row == 0 or row == self.config.rows - 1 or column == 0 or column == self.config.columns - 1):
                    self.pinned.add(index)
